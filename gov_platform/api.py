"""HTTP API boundary for the government case workflow."""
...
async def list_cases(
    queue: str | None = None,
    state: str | None = None,
    identity: ApiIdentity = Depends(require_identity),
    session: AsyncSession = Depends(get_session),
) -> list[CaseResponse]:
    states = {state} if state else None
    if state is not None and state not in CASE_STATES:
        raise HTTPException(status_code=400, detail="invalid_case_state")
    cases = await CasePersistenceRepository(session).list_queue(
        tenant_id=identity.tenant_id, queue=queue, states=states
    )
...
async def assign_case(
    ...
    identity: ApiIdentity = Depends(require_role("case_manager", "administrator")),
    session: AsyncSession = Depends(get_session),
) -> CaseResponse:
    try:
        case = await CasePersistenceRepository(session).assign(
            tenant_id=identity.tenant_id,
            case_id=case_id,
            queue=request.queue,
...
async def transition_case(
    ...
    identity: ApiIdentity = Depends(require_role("decision_officer", "case_manager", "administrator")),
    session: AsyncSession = Depends(get_session),
) -> CaseResponse:
    if request.target not in CASE_STATES:
        raise HTTPException(status_code=400, detail="invalid_case_state")
    try:
        case = await CasePersistenceRepository(session).transition(
            tenant_id=identity.tenant_id,
            case_id=case_id,
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="GoAnalyze Government API",
        version="2.1.0",
        description="Decision-support API for secure environmental case workflows.",
