...
Phase 3 adds reusable government decision-support primitives without claiming production legal compliance or authoritative Québec regulatory content.
...
- Human-controlled case state machine with explicit decision-officer approval/rejection boundary.
- Tamper-evident SHA-256 audit event chaining and verification using the existing Pydantic audit model.
...
- Workflow approval remains a human action by an explicitly identified decision-officer role; no legally binding environmental decision is automated.

## Acceptance criteria

The Phase 3 acceptance workflow must execute Phase 3 tests, the full Python test suite, Ruff, and mypy against the exact commit under test. A green result is evidence for the tested commit only.
